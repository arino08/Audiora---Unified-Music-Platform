...
public class SpotifyPlaybackController {
    ...
    public ResponseEntity<?> state(
        ...
        if (token == null) return ResponseEntity.status(401).body(
            Map.of("error", "invalid_session")
        );
        String raw = spotifyApiService
            .getPlaybackStateRaw(token, sessionId)
            .block();
    ...
    public ResponseEntity<?> playWithBody(
        @RequestHeader(
            name = "X-Session-Id",
            required = false
        ) String sessionId,
    ...
    public ResponseEntity<?> play(
        ...
        Integer code = spotifyApiService
            .resumeOrStartPlayback(token, sessionId, body)
            .block();
        return ResponseEntity.status(code == null ? 500 : code).body(
            Map.of("status", code)
        );
    }
    ...
    @PutMapping("/seek")
    public ResponseEntity<?> seek(
        @RequestHeader(
            name = "X-Session-Id",
            required = false
        ) String sessionId,
        @RequestParam(name = "position_ms") long positionMs
    ) {
        TokenInfo token = requireToken(sessionId);
        if (token == null) return ResponseEntity.status(401).body(
            Map.of("error", "invalid_session")
        ...
        Integer code = spotifyApiService
            .seekPlayback(token, sessionId, positionMs)
            .block();
        return ResponseEntity.status(code == null ? 500 : code).body(
            Map.of("status", code)
        );
    ...
    private static final Pattern SPOTIFY_TRACK_ID = Pattern.compile(
        "^[0-9A-Za-z]{22}$"
    );
    ...
    public ResponseEntity<?> playTrack(
        ...
        if (body == null) return ResponseEntity.badRequest().body(
            Map.of("error", "missing_body")
        );
        Object raw = body.get("uri");
        if (raw == null) raw = body.get("id");
        if (raw == null) return ResponseEntity.badRequest().body(
            Map.of("error", "missing_uri_or_id")
        );
        String value = raw.toString();
        String uri = value.startsWith("spotify:track:")
            ? value
            : (SPOTIFY_TRACK_ID.matcher(value).matches()
                  ? "spotify:track:" + value
                  : null);
        if (uri == null) return ResponseEntity.badRequest().body(
            Map.of("error", "invalid_track_reference")
        );
        Long positionMs = null;
        if (body.containsKey("positionMs")) {
            try {
                positionMs = Long.parseLong(body.get("positionMs").toString());
            } catch (NumberFormatException ignored) {}
        }
        String json = positionMs == null
            ? "{\"uris\":[\"" + uri + "\"]}"
            : "{\"uris\":[\"" + uri + "\"],\"position_ms\":" + positionMs + "}";
        if (deviceId != null) {
            // Pre-transfer playback if needed
            spotifyApiService
                .transferPlayback(token, sessionId, deviceId, false)
                .block();
        }
        Integer code = spotifyApiService
            .resumeOrStartPlayback(token, sessionId, json)
            .block();
        return ResponseEntity.status(code == null ? 500 : code).body(
            Map.of("status", code, "uri", uri)
        );
    }
    ...
    public ResponseEntity<?> transfer(
        ...
        if (did == null) return ResponseEntity.badRequest().body(
            Map.of("error", "missing_device_id")
        );
        boolean play = Boolean.TRUE.equals(body.get("play"));
        Integer code = spotifyApiService
            .transferPlayback(token, sessionId, did.toString(), play)
            .block();
        return ResponseEntity.status(code == null ? 500 : code).body(
            Map.of("status", code)
        );
